...
func (r *roleRepository) GetRoleList(ctx context.Context) ([]*entity.Role, error) {
	var roles []*entity.Role
...
func (r *roleRepository) GetRole(ctx context.Context, id string) (*entity.Role, error) {
	var role entity.Role
	err := r.db.WithContext(ctx).Where("col_id = ?", id).First(&role).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *roleRepository) DeleteRole(ctx context.Context, id string) error {
	return r.db.WithContext(ctx).Where("col_id = ?", id).Delete(&entity.Role{}).Error
}
